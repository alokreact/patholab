                ...
                <div class="col-lg-12">
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <!-- Horizontal Form -->
                            <form action="{{ route('admintestbyorgan.update',$organ_subtest->id) }}" method="post" enctype="multipart/form-data">
                                @csrf
                                @method('put')
                                    ...
                                    <div class="col-sm-10">
                                        <select class="form-control" name="organ">
                                            @foreach ($organs as $organ)
                                                <option value="{{ $organ->id }}" {{$organ->id === $organ_subtest->id?'selected':''}}>{{ $organ->name }}</option>
                                            @endforeach
                                        </select>
...
@push("after-scripts")
     <script>
        ...
        $('.js-example-tags').val(selectvalues).trigger('change');
        //}
